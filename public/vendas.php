...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([$produto_id, $quantidade, $valor_total, $cliente_nome, $cliente_id, $forma_pagamento, $user_id]);
        $stmt = $conn->prepare("UPDATE produtos SET quantidade = quantidade - ? WHERE id = ? AND user_id = ?");
        $stmt->execute([$quantidade, $produto_id, $user_id]);
        $conn->commit();
...
$stmt = $conn->prepare("SELECT id, nome, preco, quantidade FROM produtos WHERE user_id = ?");
    ...
    if (resultados.length > 0 && query.length > 0) {
        sugestoesProduto.innerHTML = resultados.map(p => {
            ...
            return `<div style="padding:5px; cursor:pointer;" 
                        onclick="selecionarProduto(${p.id}, '${p.nome.replace(/'/g, "\\'")}')">
                        ${p.nome} - ${preco} ${estoque}
                    </div>`;
        }).join('');
...
document.addEventListener('click', (e) => {
    if (!sugestoesCliente.contains(e.target) && e.target !== inputCliente && e.target !== inputProduto) {
        sugestoesCliente.style.display = 'none';
    }
});
</script>
